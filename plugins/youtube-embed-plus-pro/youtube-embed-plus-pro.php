                        ...
                        <div id="wizoptionboxRight">
                            ...
                            <div class="optionlist">
                                ...
                                <p>
                                    ...
                                    <label><input type="radio" ng-model="model.rel" ng-value="1">Show related videos</label>
                                    <label><input type="radio" ng-model="model.rel" ng-value="0">Show related only from the video's channel</label>
                                    <label><input type="radio" ng-model="model.rel" ng-value="-1">Hide related videos after playback <sup class="orange">new</sup></label>
                                </p>
                                <label class="opt-wizhastip">
                                    <input ng-click="showinfoChange()" <?php checked($wiz_defaults[self::$opt_showinfo], 1); ?> id="chkShowinfo" type="checkbox" name="chkShowinfo"><span class="epyt-deprecated">Show video title on hover and before playing</span>
                                    <span class="ui-icon ui-icon-info" ng-if="model.showinfo == 0"></span>
                                    <span class="tip" ng-if="model.showinfo == 0">
                                        <strong>Note:</strong> Google/YouTube no longer allows tools to control this feature. Learn more about the <a target="_blank" href="https://developers.google.com/youtube/player_parameters#Revision_History">deprecation of this feature here</a>.
                                    </span>                                    
                                </label>
        ...
        if (self::$alloptions[self::$opt_pro] && strlen(trim(self::$alloptions[self::$opt_pro])) > 9 && self::$alloptions[self::$opt_spdc] == 1 && !(bool) self::$alloptions[self::$opt_gdpr_consent])
        {
            try
            {
                ...
                if (!empty($spdcval))
                {
                    ...
                    if (preg_match($pattern_videoid, $spdckey, $init_id_matches))
                    {
...
//        }
    ...
    public static function get_html($m, $iscontent)
    {
        ...
        $code2 = '" class="__youtube_prefs__' . ($iscontent ? '' : ' __youtube_prefs_widget__') . $disptypeif .
                '"' . $voloutput . $acctitle . $galleryid_ifm_data . ' allow="autoplay; encrypted-media" allowfullscreen data-no-lazy="1" data-skipgform_ajax_framebjll=""></iframe>' . $endlb . $schemaorgoutput;
...
//            }
    ...
    public static function ytprefs_plugin_menu()
    {
        if (self::$alloptions[self::$opt_pro] && strlen(trim(self::$alloptions[self::$opt_pro])) > 0)
        {
            self::$admin_page_hooks[] = add_menu_page('YouTube Settings', 'YouTube PRO', 'manage_options', 'youtube-my-preferences', array(get_class(), 'ytprefs_show_options'), plugins_url('images/youtubeicon16.png', __FILE__), '10.000392854349');
            ...
            self::$admin_page_hooks[] = add_submenu_page('youtube-my-preferences', 'YouTube Analytics Dashboard', '<img style="width: 16px; height: 16px; vertical-align: text-top;" src="' . plugins_url('images/epstats16.png', __FILE__) . '" />&nbsp;&nbsp;PRO Analytics', 'manage_options', 'youtube-ep-analytics-dashboard', array(get_class(), 'epstats_show_options'));

            include_once(EPYTVI_INCLUDES_PATH . 'vi_admin_menu.php');
        }
        else
        {
            self::$admin_page_hooks[] = add_menu_page('YouTube Settings', 'YouTube', 'manage_options', 'youtube-my-preferences', array(get_class(), 'ytprefs_show_options'), plugins_url('images/youtubeicon16.png', __FILE__), '10.000392854349');
            self::$admin_page_hooks[] = add_submenu_page('youtube-my-preferences', '', '', 'manage_options', 'youtube-my-preferences', array(get_class(), 'ytprefs_show_options'));
        }
    ...
    public static function epstats_show_options()
    {
        ...
        <div class="wrap">
            ...
            {
                //// header
                echo "<h1>" . '<img alt="YouTube Plugin Icon" src="' . plugins_url('images/epstats16.png', __FILE__) . '" /> ' . __('YouTube Analytics Dashboard') . "</h1>";
                ?>
    ...
    public static function custom_admin_pointers()
    {
        ...
        //$new_pointer_content .= "This version provides clearer instructions for many options across the plugin&apos;s settings and wizard pages in both Free and Pro versions.";
                                ...
                                <p>
                                    ...
                                    <span id="boxschemaorg">
                                        <span class="apikey-msg">
                                            The video SEO tags include data like the title, description, and thumbnail information of each video you embed. This plugin automatically extracts this data directly from YouTube using the version 3 API. This particular API version requires that you obtain a server API key so that YouTube can authenticate the requests. <a href="https://www.youtube.com/watch?v=6gD0X76-v_g" target="_blank">Watch this video to see how to create your own key</a>. Then, paste it in the "YouTube API Key" box at the top of this screen, and click the "Save Changes" button.
                                        </span>
                                    </span>
                                </p>
                                <div class="hr"></div>
