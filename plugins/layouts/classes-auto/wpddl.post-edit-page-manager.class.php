...
class WPDD_PostEditPageManager {
	...
	private static $FORBIDDEN_ACTIONS = array( 'inline-save', 'heartbeat', 'save_layout_data_front_end' );
	...
	function meta_box( $post ) {
		...
		if ( $this->has_private_layout && $this->private_layout_in_use && $is_theme_integrated === false ) {

			$show_template_layout_selector = false;
			?>
            <p class="toolset-alert toolset-alert-warning" id="js-content-layout-in-use-message">
				<?php
				echo sprintf( __( 'You are editing this %1$s with Layouts so you cannot assign a Template Layout to it.', 'ddl-layouts' ), $post_type_obj->labels->singular_name );
				?>
            </p>
			<?php
        ...
        <div id="js_show_template_layout_selector"
            ...
            <div class="ddl-layout-selected"
                ...
                <b><?php _e( 'Selected template Layout:', 'dd-layouts' ); ?></b> <span
                        id="js_selected_layout_template_name"><?php if ( $template_selected ) {
	...
	function wpddl_save_post( $pidd ) {
		...
		if ( $_POST && isset( $_POST['action'] ) && in_array( $_POST['action'], self::$FORBIDDEN_ACTIONS ) === false ) { // Don't save in quick edit mode.
            ...
            if ( isset( $_POST['action'] ) && in_array( $_POST['action'], array( 'wcml_update_product', 'ddl_update_post_content_for_private_layout' ) ) ) {
