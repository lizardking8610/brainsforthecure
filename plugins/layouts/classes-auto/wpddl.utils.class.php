...
final class WPDD_Utils {
	...
	public static final function add_hooks() {
		...
		add_filter( 'ddl-filter_get_layout_cells_by_type', array( __CLASS__, 'filter_get_layout_cells_by_type' ), 10, 3 );
		
		add_filter( 'ddl-filter_get_cell_content_property', array( __CLASS__, 'filter_get_cell_content_property' ), 10, 3 );
	...
	public static final function get_layout_cells_by_type( $layout_id = 0, $cell_type = '' ) {
		...
		if ( 
			is_object( $layout ) 
			&& method_exists( $layout, 'get_cells_of_type' )
		) {
