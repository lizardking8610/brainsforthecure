...
DDLayout.templateSelector = function ( $ ) {
    ...
    self._stop_using_template_layout = function(){
        jQuery( '.js_ddl_stop_using_this_template_layout' ).click( function( event ) {
            ...
            jQuery.post( ajaxurl, data, function( response ) {
                ...
                // hide overlay here
                if( DDLayout_settings.DDL_JS.post.private_layout_in_use !== 'yes' ){
    ...
    self._on_layout_selector_open = function () {
        ...
        var no_results_msg = '';
        if(typeof(DDLayout_settings) !== 'undefined' && DDLayout_settings.DDL_JS.no_items_found_message ){
            no_results_msg = DDLayout_settings.DDL_JS.no_items_found_message;
        }
        ...
        if ( typeof(DDLayout_settings) !== 'undefined' && DDLayout_settings && DDLayout_settings.DDL_JS.layout !== null ) {
            $('#ddl-js-layout-template-name').append( '<option selected data-id="' + DDLayout_settings.DDL_JS.layout.id + '" value="' + DDLayout_settings.DDL_JS.layout.slug + '">' + DDLayout_settings.DDL_JS.layout.name + '</option>');
        ...
        $('#ddl-js-layout-template-name').on( 'chosen:showing_dropdown', function ( evt, params ) {
            ...
            jQuery.post(ajaxurl, data, function (response) {
                ...
                jQuery.each(json, function (key, value) {
                    ...
                    var selected = '';
                    if (
                        DDLayout_settings.DDL_JS.layout !== null &&
                        DDLayout_settings.DDL_JS.layout.id === value.template_id
                    ) {
                        selected = 'selected';
                    }
