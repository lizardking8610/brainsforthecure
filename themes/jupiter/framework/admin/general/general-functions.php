...
function mk_theme_is_masterkey() {
    ...
    if (THEME_OPTIONS_DEV_VERSION) {
        $theme_options_slugs[] = 'theme_options_' . THEME_OPTIONS_DEV_VERSION;
    }
...
function mk_is_control_panel() {
    if (isset($_GET['page']) && in_array($_GET['page'], array('Jupiter', 'theme-plugins' , 'theme-templates','theme-addons' , 'theme-image-size','theme-status','icon-library', 'theme-support', 'theme-updates', 'theme-announcements'))) {
        return true;
    }
    return false;
}
