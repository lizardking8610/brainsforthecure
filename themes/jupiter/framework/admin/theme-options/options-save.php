...
class mk_theme_options {
	...
	public function save_theme_options( $theme_options_data, $save_revision = true ) {
		...
		if ( empty( $theme_options_data ) ) {
			$response = [ 'msg' => 'Not saved' , 'status' => false , 'data' => 'mk-not-saved'];
		}
		...
		if ( update_option( THEME_OPTIONS, $theme_options_data ) && update_option( 'mk_jupiter_flush_rules', 1 ) ) {
			...
			$response = [ 'msg' => 'Successfull' , 'status' => true , 'data' => 'mk-success-save'];
		} else {
			$response = [ 'msg' => 'Already saved' , 'status' => false , 'data' => 'mk-already-saved'];
		}
