...
class mk_plugin_management {
	...
	public function mk_plugin_list_handler() {
		try {
			...
			$this->plugins_list_from_api( $from , $count , $exclude_plugins );
		} catch (Exception $e) {
		...
		try {
			...
			if ( ($plugin_path = $this->find_plugin_path( $this->get_plugin_slug() )) !== false ) {
				if ( $this->check_active_plugin( $this->get_plugin_slug() ) == true ) {
					$this->message( $this->adminpanel_text( 'plugin_is_successfully_installed_activated' ), true );
					return true;
				}
			...
			if ( is_array( $api_response ) == false || count( $api_response ) < 1 ) {
				$message = $this->adminpanel_text( 'plugin_param_not_exist' );
			...
			if ( filter_var( $api_response[0]['source'], FILTER_VALIDATE_URL ) === false ) {
				throw new Exception( $this->adminpanel_text( 'plugin_source_not_found_or_invalid' ) );
			}
			...
			if ( $found_file === false ) {
				$need_to_be_replace = $this->adminpanel_text( 'plugin_path_is_not_found' );
	...
	public function update_plugin() {
		try {
			...
			if ( $plugin_path === false ) {
				throw new Exception( $this->adminpanel_text( 'plugin_head_is_not_found' ) );
			}
			...
			$this->set_lock( 'update_plugin' , false )->message( $this->adminpanel_text( 'plugin_is_successfully_updated' ), true );
			return true;
		} catch (Exception $e) {
			$this->message( $e->getMessage(), false );
			return false;
	...
	public function plugins_list_from_api( $from = 0, $count = 1, $exclude_plugins = array() ) {
		...
		if ( empty( $response->body->data ) ) {
			$this->message( $this->adminpanel_text( 'successfull' ), true, array() );
			return true;
		}
		...
		$this->message( $this->adminpanel_text( 'successfull' ), true, $result );
		return true;
	}
	...
	public function list_of_installed_plugin() {
		try {
			...
			if ( is_array( $list_of_plugins ) && count( $list_of_plugins ) > 0 ) {
				...
				$this->message( $this->adminpanel_text( 'successfull' ), true, $list_of_plugins );
				return true;
			} else {
				$this->message( $this->adminpanel_text( 'plugin_list_empty' ), false );
				return false;
			}
		} catch (Exception $e) {
			$this->message( $e->getMessage(), false );
	...
	public function install_batch( $plugins_slug_list ) {
		try {
			if ( empty( $plugins_slug_list ) || is_array( $plugins_slug_list ) == false || count( $plugins_slug_list ) == 0 ) {
				throw new Exception( $this->adminpanel_text( 'plugin_list_not_array' ) );
			}
			...
			$message_need_to_replace = $this->adminpanel_text( 'plugin_is_successfully_installed_bulk' );
	...
	public function precheck() {
		...
		if ( is_writable( $this->get_plugins_dir() ) == false ) {
				throw new Exception( $this->adminpanel_text( 'plugin_directory_not_writable' ) );
				return false;
		}
	}
	/**
	...
	public function activate_plugin( $plugin_slug ) {
		...
		if ( $plugin_path === false ) {
			throw new Exception( $this->adminpanel_text( 'plugin_path_is_not_found_in_active_list' ) );
			return false;
		}
	...
	public function remove_plugin( $plugin_slug ) {
		...
		try {
			if ( is_writable( $this->get_plugins_dir() ) == false ) {
				throw new Exception( $this->adminpanel_text( 'plugin_directory_not_writable_rppm01x01' ) );
			}
			if ( ($plugin_path = $this->find_plugin_path( $plugin_slug )) == false ) {
				throw new Exception( $this->adminpanel_text( 'plugin_remove_not_exist_rppm01x01' ) );
			}
			...
			if ( strlen( str_replace( $this->get_plugins_dir(), '', $plugin_full_path ) ) > 2 ) {
				if ( is_writable( $plugin_base_directory ) == false ) {
					throw new Exception( $this->adminpanel_text( 'plugin_directory_not_writable_rppm01x01' ) );
				}
				if ( Abb_Logic_Helpers::deleteFileNDir( $plugin_base_directory ) ) {
					$this->message( $this->adminpanel_text( 'plugin_deactivate_successfully' ), true );
					return true;
				} else {
					throw new Exception( $this->adminpanel_text( 'plugin_directory_not_removable_rppm01x01_directory' ) );
				}
			} else {
				if ( Abb_Logic_Helpers::deleteFileNDir( $plugin_full_path ) ) {
					...
					throw new Exception( $this->adminpanel_text( 'plugin_directory_not_removable_rppm01x01_file' ) );
				}
			}
	...
	/**
	 * this method is resposible to get dynamic admin panel text
	 *
	 * @author Sofyan Sitorus <sofyan@artbees.net>
	 *
	 * @param str   $key key of texts array
	 * @param str   $text
	 */
	private function adminpanel_text($key){
		$text = mk_adminpanel_textdomain( 'plugin-management' );
