...
<div class="control-panel-holder">
    ...
    <div class="abb-premium-templates cp-pane">
        ...
        if ( $compatibility->checkErrorExistence() === true ) {
            echo mk_get_control_panel_view('register-product-popup', true, array('message' => sprintf(__('In order to install new templates you must resolve compatibility issues first. %s' , 'mk_framework') , '<br><a target="_blank" href="https://themes.artbees.net/docs/server-requirements-jupiter/">Learn more</a>')));
            $error_flag = true;
        }
        if (
            get_filesystem_method() !== 'direct' || (isset($GLOBALS['_wp_filesystem_direct_method']) && $GLOBALS['_wp_filesystem_direct_method'] != 'file_owner')) {
            echo mk_get_control_panel_view('register-product-popup', true, array('message' => sprintf(__('Whoops! There seems to be an inconsistency in your server. Don’t panic though, <br> Here we’ve listed some possible solutions for you to get back on track. %s' , 'mk_framework') , ' <a target="_blank" href="https://themes.artbees.net/docs/installing-template/">Learn more</a>')));
            $error_flag = true;
        }
        if ( $error_flag ) {
            ?>
